...
If any query params were passed as part of the request URL, they can
be accessed in a Clojure map stored in a namespace-global variable
called "request-params".

Anything that your script writes to standard out (e.g., using "print"
or "println") will be collected and returned to the client as the
contents of a text/gemini document (metadata: 20 text/gemini;
charset=utf-8). Please refer to the [[Gemtext Markup]] section below for
details on this syntax.

While supporting ad-hoc CLJ scripts in this way provides users with a
great deal of flexibility in building out dynamic Gemini applications
in Clojure, it is certainly not the most secure approach in the world.
Keep in mind that all CLJ scripts run with the same permissions as the
server process, so server administrators are encouraged to run Space
Age as an unprivileged user whenever it is deployed within a
multi-user context.
