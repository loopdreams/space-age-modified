...
update the values assigned to these two attributes under the
=:jvm-opts= settings in the =:run-server= alias:
...
the variable definitions within it to point to your =keystore.pkcs12=
and =space-age-<version>.jar= files.
